# =========================================================================
# Project: Bayesian t-Hidden Markov Model for Financial Risk
# Description: Implementation of a custom MCMC sampler in NIMBLE to 
#              estimate regime-switching and forecast VaR/ES on tech stocks.
# Objective:   Quantitative tail-risk forecasting and market regime detection
# =========================================================================

# ====================================================================
# 1. DATA ACQUISITION & PREPROCESSING
# ====================================================================
# Fetching market data and computing log-returns to ensure stationarity 
# before feeding them into the Hidden Markov Model.
...
# ====================================================================
# 2. BAYESIAN t-HMM SPECIFICATION (NIMBLE)
# ====================================================================
# Defining the hierarchical model. A Student's t-emission distribution 
# is constructed using a Normal-Inverse-Gamma scale mixture to robustly 
# capture heavy tails and extreme market events (crashes/rallies).
...
tHMM_code <- nimbleCode({
  ...
  # Priors for transition probabilities between market regimes
  for(u in 1:K) {
  ...
  # Priors for state-specific precision matrices (Wishart)
  for(u in 1:K) {
    Sigma_inv[u, 1:D, 1:D] ~ dwish(R = R_prior[1:D, 1:D], df = alpha_prior)
    Sigma[u, 1:D, 1:D] <- inverse(Sigma_inv[u, 1:D, 1:D])
  }
  
  # Prior for degrees of freedom (nu controls the heavy-tailedness)
  ...
  # Scaling variables (Gamma parameters for the t-distribution mixture)
  for(t in 1:T) {
...
# ====================================================================
# Using K-means clustering to provide smart initial values to the MCMC.
# This prevents the chain from getting stuck in local optima and 
# drastically reduces the required burn-in period.
...
# ====================================================================
# 4. CUSTOM MCMC SAMPLER
# ====================================================================
# A custom Random Walk Metropolis-Hastings step on a logarithmic scale.
# Standard samplers often fail to efficiently explore the parameter 
# space for degrees of freedom (nu); the log-transformation ensures positivity.
...
sampler_nu_logRW <- nimbleFunction(
    ...
    # Likelihood of gamma variables dependent on nu
  ...
  run = function() {
    ...
    # Log acceptance ratio (Prior + Likelihood + Jacobian correction)
...
run_tHMM_MCMC <- function(Y, K = 4, niter = 20000, nburnin = 2000,
  ...
  # Inject the custom log-RW sampler for nu
...
# ====================================================================
# 6. LABEL-SWITCHING CORRECTION
# ====================================================================
# Corrects label-switching by ordering states according to the variance 
# of a specific constraint dimension (e.g., S&P 500 variance).
# Essential to maintain economic interpretability (e.g., State 1 = Bull Market, 
# State K = Crisis/High Volatility).
...
relabel_samples <- function(samples, K, D, constraint_dim = 6) {
  ...
  for(iter in 1:n_samples) {
    ...
    if(!all(ord == 1:K)) {
      ...
      # Reorder Covariance and Precision Matrices
      for(k in 1:K) {
        for(i in 1:D) {
          for(j in 1:D) {
      ...
      # Reorder Initial Probabilities pi
      for(k in 1:K) {
...
# ====================================================================
# 7. POSTERIOR INFERENCE & SUMMARIZATION
# ====================================================================
...
get_posterior_estimates <- function(samples, K, D, var_names = NULL) {
  ...
  if(!is.null(var_names)) colnames(estimates$mu) <- var_names
  ...
  # Posterior Initial Probabilities
  ...
  # Posterior State Extraction (Mode)
...
# ====================================================================
# 8. QUANTITATIVE RISK FORECASTING (VaR & ES)
# ====================================================================
# Monte Carlo simulation of future return paths based on MCMC posterior draws.
# This explicitly computes Value at Risk (VaR) and Expected Shortfall (ES) 
...
forecast_tHMM <- function(mcmc_output, h = 10, n_samples = 1000) {
  ...
  for(s in 1:n_samples) {
    ...
    for(t_ahead in 1:h) {
      ...
      Y_forecast[s, t_ahead, ] <- mvrnorm(n = 1, mu = mu[U_next, ], Sigma = Sigma_scaled)
      U_current <- U_next
    }
  }
...
# ====================================================================
# Running the complete pipeline: MCMC -> Relabeling -> Metrics -> Risk
...
# Apply constraint to solve label-switching
...
traceplot_trimmed_mu <- function(samples_mcmc, drop_frac = 0.10, n_plot = 28, main_title = NULL) {  
  mat <- as.matrix(samples_mcmc)
  n <- nrow(mat) 
  start <- floor(n * drop_frac) + 1 
  ...
  op <- par(mfrow = c(4, 7), mar = c(2.5, 2.5, 2, 1), oma = c(0, 0, 2, 0)) 
